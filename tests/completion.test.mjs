...
{
  let log = applyMeasurement(fresh(), 't1', 42, AT, 'pair:A|D');
...
{
  let log = applyMeasurement(fresh(), 't1', 0, AT);
...
{
  let log = applyMeasurement(fresh(), 't1', 40, AT);
  log = applyMeasurement(log, 't1', 31, AT + 60_000);
  check('the better number stands', log.drillResults.t1 === 40);
  check('and the day says when it was last run', recordFor(log, 't1').at === AT + 60_000);

  const silent = applyMeasurement(log, 't1', 0, AT + 120_000);
  check('a later silent run cannot erase what was heard',
    recordFor(silent, 't1').evidence === 'measured');
  check('and cannot delete the number either', silent.drillResults.t1 === 40);
}

console.log('\nEvery run, not just the best one\n');
{
  let log = applyMeasurement(fresh(), 't1', 38, AT);
  log = applyMeasurement(log, 't1', 41, AT + 60_000);
  log = applyMeasurement(log, 't1', 39, AT + 120_000);
  const runs = runsFor(log, 't1');
  ...
  check('while the day still summarises to its best', log.drillResults.t1 === 41);

  const silent = applyMeasurement(log, 't1', 0, AT + 180_000);
  check('a run nothing was heard in is not counted as a run',
    runsFor(silent, 't1').length === 3);
...
{
  let log = applyMeasurement(fresh(), 't1', 44, AT);
...
console.log('\nThe player correcting the record\n');
{
  ...
  const measured = settle(applyMeasurement(fresh(), 't1', 40, AT), 't1');
  ...
  check('but never deletes what was heard', cleared.drillResults.t1 === 40);
...
console.log('\nA day recorded before any of this existed\n');
{
  ...
  const touched = settle(applyMeasurement(old, 't1', 25, AT), 't1');
  check('practising again today still works on it', touched.drillResults.t1 === 25);
